...
<head>
    ...
    <title>Nova Seleção – Copa do Mundo</title>
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card h2 { color: #004d26; font-size: 1.4rem; margin-bottom: 28px; }
        ...
        .btn-save   { background: #FFD700; color: #003d1f; }
...
<main>
    <div class="card">
        <h2><span>⚽</span>Nova Seleção</h2>

        <?php if (!empty($erro)): ?>
            <div class="alert-error">⚠️ <?= htmlspecialchars($erro) ?></div>
        <?php endif; ?>

        <form method="POST" action="/selecoes/salvar">
            ...
            <div class="form-group">
                ...
                <select id="grupo" name="grupo" required>
                    ...
                    $grupoAtual = strtoupper($_POST['grupo'] ?? '');
                    foreach ($grupos as $g):
                    ?>
                        <option value="<?= $g ?>" <?= $grupoAtual === $g ? 'selected' : '' ?>><?= $g ?></option>
                    <?php endforeach; ?>
                </select>
                <small>Grupos de A a H</small>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="participacoes" name="participacoes" min="0" max="30"
                    placeholder="0"
                    value="<?= (int)($_POST['participacoes'] ?? 0) ?>">
            ...
            <div class="form-group">
                ...
                <input type="number" id="titulos" name="titulos" min="0" max="10"
                    placeholder="0"
                    value="<?= (int)($_POST['titulos'] ?? 0) ?>">
                <small>Número de títulos mundiais conquistados</small>
            </div>

            <div class="btn-row">
                <button type="submit" class="btn btn-save">💾 Salvar Seleção</button>
